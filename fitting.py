...
def plot_2d(ff, lim=True):
    ...
    dimer_ccsdt = pd.read_csv("data/dimers_ccsdt.csv")
    len(dimer_ccsdt)
    ...
    ff.data = dimer_ccsdt
    _ = ff.get_best_df()
    dist = [0.75, 1, 1.25, 1.5, 1.75, 2, 2.25, 2.5, 2.75, 3, 3.25, 3.5, 3.75, 4, 4.25, 4.5, 4.75, 5, 5.25, 5.5, 5.75, 6,
            7, 8]
...
cluster_dict = {_.name: [x for x in _.iterdir() if x.name.endswith("out") and x.name.startswith("t")] for _ in
                cluster_files}
singles_dict = {_.name: [x for x in _.iterdir() if x.name.endswith("out") and x.name.startswith("t")] for _ in
                singles_files}
...
def prepare_FF(df, func, elec="ele", data="int_CCSDT"):
    cluster = df.copy()
    ...
    ff = FF(df, {"OO": OO_np, "OH": OH_np, "HH": HH_np}, func, elec=elec)
    return ff


elec = "ELST"  # exact electrostatics
ff = prepare_FF(cluster_mdcm, get_DEplus)
